...
def function13():
    ...
    condition = np.logical_and(arr>5,arr<10)
    ans = np.extract(condition, arr)#write your code here 
    return ans
...
#task14
def function14():
    # Create an 8X3 arr[arr>5]  arr[arr<10[arr>5]  arr[arr<10teger array from a range between 10 to 34 such that the difference between each element is 1 and then Split the array into four equal-sized sub-arrays.
...
def function18():
    ...
    x = np.sum(np.isin(students, piaic))# Write you code Here
